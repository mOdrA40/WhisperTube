    ...
    <div className="transcript-card card">
      <div className="transcript-toolbar">
        ...
        <div className="toolbar-actions">
          <button className="secondary-button" onClick={onCopy}>{copied ? <Check size={16} /> : <Copy size={16} />}{copied ? "Copied" : "Copy"}</button>
          <button className="secondary-button" onClick={() => onExport("txt")}><FileText size={16} /> TXT</button>
          <button className="secondary-button" onClick={() => onExport("srt")}>SRT</button>
          <button className="secondary-button" onClick={() => onExport("vtt")}>VTT</button>
        </div>
      </div>
      <div className="transcript-search"><Search size={16} /><input value={searchQuery} onChange={(event) => onSearchChange(event.target.value)} placeholder="Cari di transcript…" /><span>{filteredSegments.length} segments</span></div>
